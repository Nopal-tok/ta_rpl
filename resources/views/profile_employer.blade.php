...
<body>
        ...
        <div class="profile-card card shadow-sm border-0">
            <div class="card-body">
                ...
                <div class="profile-header d-flex align-items-center flex-wrap">
                    ...
                    <div class="pic-name d-flex align-items-center mb-3 mb-md-0">
                        ...
                        <div class="bg-light rounded-circle overflow-hidden me-3"
                            style="width: 72px; height: 72px; position: relative; cursor: pointer;">

                            <!-- Preview Foto -->
                            <img id="companyImagePreview"
                                src="{{ $company->logo ? asset('storage/'.$profile->foto) : asset('img/default.png') }}"
                                alt="Company Logo"
                                class="w-100 h-100"
                                style="object-fit: cover;">

                            <!-- Hidden File Input -->
                            <form id="companyImageForm" action="/company/upload-logo" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="file" name="logo" id="companyImageInput" accept="image/*" style="display: none;">
                            </form>
                        </div>

                        <!-- Nama Perusahaan -->
                        <div class="text-center">
                            <p class="highlight-name fw-medium text-white">{{ $company->nama_perusahaan ?: '-' }}</p>
                        </div>
                    ...
                    <div class="d-flex flex-column text-md-end text-white contact-box">
                        <span class="d-flex align-items-center justify-content-md-start mb-2">
                            ...
                            <span class="ms-2">{{ $company->email_perusahaan ?? '-' }}</span>
                        </span>
                        ...
                        <span class="d-flex align-items-center justify-content-md-start">
                            ...
                            <span class="ms-2">{{ $company->nomor_telepon ?? '-' }}</span>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="about-section">
                <div class="form-control company-about-text">
                    <p>{{ $company->about ?? 'No about information yet.' }}</p>
                </div>
            </div>
            ...
            <div class="personal-information row g-4">
                 ...
                 <div class="col-12 col-md-4">
                    <p class="text-secondary fw-medium mb-1 small">Company Name</p>
                    <p class="fw-normal data-value mb-4">{{ $company->nama_perusahaan ?? '-' }}</p>
                </div>

                <div class="col-12 col-md-4">
                    <p class="text-secondary fw-medium mb-1 small">Email</p>
                    <p class="fw-normal data-value mb-4">{{ $company->email_perusahaan ?? '-' }}</p>
                </div>

                <div class="col-12 col-md-4">
                    <p class="text-secondary fw-medium mb-1 small">Country</p>
                    <p class="fw-normal data-value mb-4">{{ $company->negara ?? '-' }}</p>
                </div>

                <div class="col-12 col-md-4">
                    <p class="text-secondary fw-medium mb-1 small">Address</p>
                    <p class="fw-normal data-value mb-4">{{ $company->alamat_perusahaan ?? '-' }}</p>
                </div>

                <div class="col-12 col-md-4">
                    <p class="text-secondary fw-medium mb-1 small">Phone</p>
                    <p class="fw-normal data-value mb-4">{{ $company->nomor_telepon ?? '-' }}</p>
                </div>
                
            </div>
            <a href="/edit_profile_employer" class="btn-edit d-flex align-items-center justify-content-center shadow mt-5">
    ...
    <script>
        // Klik gambar logo → buka file input
        document.getElementById('companyImagePreview').onclick = function() {
            document.getElementById('companyImageInput').click();
        };

        // Setelah pilih file → preview + autosubmit
        document.getElementById('companyImageInput').onchange = function() {
            if (this.files && this.files[0]) {
                ...
                // submit otomatis
                document.getElementById('companyImageForm').submit();
            }
        };
    </script>
